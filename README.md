...
It starts by looking for the pdf you name in the ```data/pdfs``` directory. If it's not there, it'll download it from the [State Police's website](http://www.njsp.org/info/ucr_currentdata1.html?agree=0). If it is there, it splits it into separate, single-page PDFs (using the [docsplit](https://documentcloud.github.io/docsplit/) Ruby gem) and puts them into the ```data/burstpdfs``` directory. For each pdf there, it uses [tabula-extractor](https://github.com/tabulapdf/tabula-extractor) to pull out the data table and spit it into a csv. All of the csvs wind up in the aptly named ```data/csvs``` directory.
